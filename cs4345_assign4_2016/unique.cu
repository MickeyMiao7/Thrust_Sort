...
// COMMENTS TO GRADER:
// In this assignment, I completed the implementation of a function and two CUDA kernels to compute a sorted version of an input integer array with all the duplicate elements removed.

// For the Kernel_MarkUnique() function, I have implemented two kinds of solutions.

// The first is to use a shared memory of BLOCK_SIZE to store all the elements in the block. 
// For each element in block, check whether  sharedArray[tx - 1] == sharedArray[tx]. 
// But for the first element, I need to check whether sharedArray[0] == inSortedArray[tid - 1], which means I need to do another read operation from global memory

// The second solution is that I use a shared memory of BLOCK_SIZE + 1 to store all the element of the block and, with another element which is before the first element of the block. 
// But in this situation, I meet the problem of bank conflict of reading shared memory because for the first and last element of the block they will read from the same bank.

// I consulted the professor. It seems that the first solution is better so I make the second one commented. It can be seen as my thinking process.


// I compiled and run my program successfully on the computer in Multi Media Lab 2A. If any problems occur, please do not hesitate to contact with me since I have put lots of efforts.
// Thanks sincerely.
// ============================================================
//
// FILE: unique.cu
                                   ...
                                   int numElems )
{
	...
	// The first solution of this funcition is to use a shared memory of BLOCK_SIZE to store all the elements in the block. 
	// For each element in block, check whether  sharedArray[tx - 1] == sharedArray[tx]. 
	// But for the first element, I need to check whether sharedArray[0] == inSortedArray[tid - 1], which means I need to do another read operation from global memory
	...
	if ( tid < numElems )
		sharedData[tx] = inSortedArray[tid]; 
		
		// synchronize to avoid conflicts
		__syncthreads();

		if ( tid == 0 )
			outSelectionArray[0] = 1;
		else
		{
			// The first element of each block needs to compare with the last element of the previous block
			if ( tx == 0 )
				outSelectionArray[tid] = sharedData[tx] == inSortedArray[tid - 1] ? 0 : 1;
			else
				outSelectionArray[tid] = sharedData[tx] == sharedData[tx - 1] ? 0 : 1;
		}
	...
	__shared__ int sharedData[BLOCK_SIZE + 1];

	int index = tx + 1;
	if ( tid < numElems )
	{	
		sharedData[index] = inSortedArray[tid];
		if ( tid > 0 && tx == 0 ){
			sharedData[0] = inSortedArray[tid - 1];
		}
		__syncthreads();

		if ( tid > 0 )
			outSelectionArray[tid] = sharedData[index] == sharedData[tx] ? 0 : 1;
		else 
...
//---------------------------------------------------------------------------
	...
	// Step (3) Scan selectionArray and get scatterAddressArray
