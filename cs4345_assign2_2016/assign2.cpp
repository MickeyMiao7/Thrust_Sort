...
// COMMENTS TO GRADER:
// In this assignment, I completed the odd-even transition sort on GPU by applying the ping-pong technique and got the correct answer which is the same as the one computed by CPU.

// In the part of assign2.cpp, I think there are not too many codes to finish. I set the relevant funcation and their parameters as the example shown by the professor.
...
//-----------------------------------------------------------------------------
	...
	// The size of projection matrix and view matrix is associated to the size of the array, which is texWidth * texHeight
...
//-----------------------------------------------------------------------------
    ...
    //---------------------------------------------------------
		...
		// The output size is associated with the array size, which is texWidth * texHeight
		glBegin( GL_QUADS );
			glVertex2f( 0, 0 );
